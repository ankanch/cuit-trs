...
#2维散点图
#颜色随机
def  Scatterplot(xLabel,yLabel,xvaluelist,yvaluelist,graphicTitle="图例"):
    colors = numpy.random.rand(len(xvaluelist))
    #area = numpy.pi * (15 * numpy.random.rand(N))**2  # 0 to 15 point radii
    #plt.scatter(x, y, s=area, c=colors, alpha=0.5)
    plt.scatter(xvaluelist, yvaluelist, c=colors, alpha=0.5)
    #plt.scatter(xvaluelist, yvaluelist, alpha=0.5)
    plt.xlabel(xLabel,fontproperties=font_set)
    plt.ylabel(yLabel,fontproperties=font_set)
    plt.title(graphicTitle,fontproperties=font_set,y=1.05)
    plt.show()
...
#2维散点图,传入list，绘制聚类结果
#颜色固定，仅用于[用户名,[peak,peak-index]]结构
def  ScatterplotS(xLabel,yLabel,clusterlist,centerlist,graphicTitle="图例"):
    #抽取结构
    colors = ['b','g','r','c','m','y','k']
    ci = 0
    i = 0
    for plist in clusterlist:
        px = []
        py = []
        centerp = []
        for user in plist:
            ...
            px.append(user[1][1])
            py.append(user[1][0])
        centerp = centerlist[i]
        plt.scatter(px, py, c=colors[ci], alpha=0.5,s=24)
        plt.scatter(centerp[1], centerp[0], c=colors[ci], alpha=0.5,s=150)
        ci+=1
        i+=1
        del px[:]
        del py[:]
